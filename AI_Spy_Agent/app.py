...
def send_to_discord(message):
    ...
    if not DISCORD_WEBHOOK:
        gr.Warning("Discord webhook is missing. Check your .env file.")
        return False

    if not message or not message.strip():
        gr.Warning("Cannot send an empty message.")
        return False
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            gr.Info("Sent to Discord!")
            return True

        gr.Warning(f"Discord error: {response.status_code}")
        print(response.text)
        return False

    except requests.RequestException as error:
        gr.Warning(f"Discord request failed: {error}")
        return False
